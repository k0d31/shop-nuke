...
class ThreadDelete(commands.Cog):
    ...
    async def on_thread_delete(self, thread):
        ...
        logs = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.thread_delete).flatten()
        ...
        embed.add_field(name="Действие", value=f"Удаление треда - `{thread.name}`", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за удаление треда - `{thread.name}`",
            color=disnake.Color.red()
        )
        ...
        if (roles_data and roles_data[0][15] == 'false') or (user_data and user_data[0][15] == 'false'):
            await add_to_quarantinenotmember(self.bot, guild, author, embed, embeduser, quarantine_role, booster_role, data_settings)
            return

        ban_value = None
        if roles_data:
            ban_value = roles_data[0][15]
        elif user_data:
            ban_value = user_data[0][15]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Удаление треда - `{thread.name}`. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
                await warning_channel.send(embed=warning_embed)

def setup(bot):
    bot.add_cog(ThreadDelete(bot))
