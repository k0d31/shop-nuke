...
class ChannelUpdate(commands.Cog):
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        entry = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.channel_update).flatten()
        ...
        embed.add_field(name="Действие", value=f"Изменение канала `{before.name}`", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за изменение канала - `{before.name}`",
            color=disnake.Color.red()
        )
        ...
        if not roles_data and not user_data:
            ...
            await self.revert_channel_changes(after, before)
            return

        if (roles_data and roles_data[0][4] == 'false') or (user_data and user_data[0][4] == 'false'):
            await add_to_quarantinenotmember(self.bot, guild, author, embed, embeduser, quarantine_role, booster_role, data_settings)
            await self.revert_channel_changes(after, before)
            return
        ...
        ban_value = None
        if roles_data:
            ban_value = roles_data[0][4]
        elif user_data:
            ban_value = user_data[0][4]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Изменение канала - `{before.name}`. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
                await warning_channel.send(embed=warning_embed)

    async def revert_channel_changes(self, after, before):
        """Отменяет изменения в канале, вернув его к предыдущему состоянию."""
        ...
        elif isinstance(before, disnake.VoiceChannel):
            await after.edit(
                name=before.name,
