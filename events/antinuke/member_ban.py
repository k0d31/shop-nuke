...
class MemberBan(commands.Cog):
    ...
    async def on_member_ban(self, guild, member):
        ...
        embed.add_field(name="Действие", value=f"Выдача бана участнику - {member.mention}", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за блокировку участника - {member.mention}",
            color=disnake.Color.red()
        )
        ...
        if not roles_data and not user_data:
            ...
            await self.unban_member(guild, member)
            return

        if (roles_data and roles_data[0][7] == 'false') or (user_data and user_data[0][7] == 'false'):
            await add_to_quarantine(self.bot, guild, author, member, embed, embeduser, quarantine_role, booster_role, data_settings)
            await self.unban_member(guild, member)
            return
        ...
        ban_value = None
        if roles_data:
            ban_value = roles_data[0][7]
        elif user_data:
            ban_value = user_data[0][7]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Блокировка участника - {member.mention}. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
                await warning_channel.send(embed=warning_embed)

    async def unban_member(self, guild, member):
        try:
            await member.unban(reason='antinuke')
        except:
            await guild.unban(member, reason='antinuke')
