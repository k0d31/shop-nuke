...
class RoleUpdate(commands.Cog):
    ...
    async def on_guild_role_update(self, before, after):
        ...
        entry = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.role_update).flatten()
        ...
        embed.add_field(name="Действие", value=f"Изменение роли - {before.mention}", inline=False)

        embeduser = disnake.Embed(
            title = "Вы были отправлены в карантин",
            description = f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за изменение роли - `{before.name}`",
            color = disnake.Color.red()
        )
        ...
        if not roles_data and not user_data:
            ...
            await after.edit(
                ...
                icon=before.icon if hasattr(before, 'icon') else None,
                position=before.position
            )
            return
        
        
        if (roles_data and roles_data[0][12] == 'false') or (user_data and user_data[0][12] == 'false'):
            await add_to_quarantinenotmember(self.bot, guild, author, embed, embeduser, quarantine_role, booster_role, data_settings)
            await after.edit(
                name=before.name,
                permissions=before.permissions,
        ...
        ban_value = None
        if roles_data:
            ban_value = roles_data[0][12]
        elif user_data:
            ban_value = user_data[0][12]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Изменил роль - {before.mention} {remaining_warnings}/{ban_value}", inline=False)
