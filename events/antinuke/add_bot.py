...
class AddBot(commands.Cog):
    ...
    async def on_member_join(self, member):
        if member.bot:
            ...
            entry = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.bot_add).flatten()
            ...
            quarantine_data = await db.get_userinquarantine(guild.id, author.id)
            if quarantine_data:
                return
            ...
            embed.add_field(name="Действие", value=f"Добавление бота - {member.mention}", inline=False)

            embeduser = disnake.Embed(
                title="Вы были отправлены в карантин",
                description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за добавление бота - {member.mention}",
                color=disnake.Color.red()
            )
            ...
            if (roles_data and roles_data[0][10] == 'false') or (user_data and user_data[0][10] == 'false'):
                await add_to_quarantine(self.bot, guild, author, member, embed, embeduser, quarantine_role, booster_role, data_settings)
                await guild.ban(member, reason='antinuke')
                return
            ...
            ban_value = None
            if roles_data:
                ban_value = roles_data[0][10]
            elif user_data:
                ban_value = user_data[0][10]
            ...
            if self.warning_counts[author.id] >= ban_value:
                ...
                if warning_channel:
                    ...
                    warning_embed.add_field(name="Действие", value=f"Добавил бота - {member.mention}. {remaining_warnings}/{ban_value} предупреждений осталось.", inline=False)
