...
class MemberKick(commands.Cog):
    ...
    async def on_member_remove(self, member):
        ...
        entry = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.kick).flatten()
        ...
        embed.add_field(name="Действие", value=f"Кик участника - {member.mention}", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за кик участника - {member.mention}",
            color=disnake.Color.red()
        )
        ...
        if (roles_data and roles_data[0][6] == 'false') or (user_data and user_data[0][6] == 'false'):
            await add_to_quarantine(self.bot, guild, author, member, embed, embeduser, quarantine_role, booster_role, data_settings)
            return

        ban_value = None
        if roles_data:
            ban_value = roles_data[0][6]
        elif user_data:
            ban_value = user_data[0][6]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Кик участника - {member.mention}. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
                await warning_channel.send(embed=warning_embed)

def setup(bot):
    bot.add_cog(MemberKick(bot))
