...
class RoleCreate(commands.Cog):
    ...
    async def on_guild_role_create(self, role):
        ...
        entry = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.role_create).flatten()
        ...
        embed.add_field(name="Действие", value=f"Создание роли - `{role.name}`", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за создание роли - `{role.name}`",
            color=disnake.Color.red()
        )
        ...
        if not roles_data and not user_data:
            ...
            await role.delete(reason='antinuke')
            return

        if (roles_data and roles_data[0][11] == 'false') or (user_data and user_data[0][11] == 'false'):
            await add_to_quarantinenotmember(self.bot, guild, author, embed, embeduser, quarantine_role, booster_role, data_settings)
            await role.delete(reason='antinuke')
            return
        ...
        ban_value = None
        if roles_data:
            ban_value = roles_data[0][11]
        elif user_data:
            ban_value = user_data[0][11]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Создание роли - `{role.name}`. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
