...
class WebhookManagement(commands.Cog):
    ...
    async def on_webhooks_update(self, channel):
        ...
        logs = await guild.audit_logs(limit=1, action=disnake.AuditLogAction.webhook_create).flatten()
        create_entry = logs[0] if logs else None

        if not create_entry:
            return

        log_creation_time = create_entry.created_at

        now = datetime.datetime.now(datetime.timezone.utc)
        if now - log_creation_time > datetime.timedelta(minutes=1):
            return

        await self.handle_webhook_create(create_entry, guild, data_settings, quarantine_role, booster_role)


    async def handle_webhook_create(self, entry, guild, data_settings, quarantine_role, booster_role):
        ...
        embed.add_field(name="Действие", value=f"Создание вебхука - `{webhook.name}`", inline=False)

        embeduser = disnake.Embed(
            title="Вы были отправлены в карантин",
            description=f"{author.mention}, вы были отправлены в карантин на сервере {guild.name} за создание вебхука - `{webhook.name}`",
            color=disnake.Color.red()
        )
        ...
        if not roles_data and not user_data:
            ...
            await webhook.delete(reason='antinuke')
            return

        if (roles_data and roles_data[0][14] == 'false') or (user_data and user_data[0][14] == 'false'):
            await add_to_quarantinenotmember(self.bot, guild, author, embed, embeduser, quarantine_role, booster_role, data_settings)
            await webhook.delete(reason='antinuke')
            return
        ...
        ban_value = None
        if roles_data:
            ban_value = roles_data[0][14]
        elif user_data:
            ban_value = user_data[0][14]
        ...
        if self.warning_counts[author.id] >= ban_value:
            ...
            if warning_channel:
                ...
                warning_embed.add_field(name="Действие", value=f"Создание вебхука - `{webhook.name}`. Осталось предупреждений: {remaining_warnings}/{ban_value}", inline=False)
                await warning_channel.send(embed=warning_embed)


def setup(bot):
    bot.add_cog(WebhookManagement(bot))
